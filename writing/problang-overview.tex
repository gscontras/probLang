...
In the generics model, \cite{tesslergoodman2019} employe two techniques: one had participants freely generate categories and then rat the prevalence of a target feature for those categories; another had participants answer targeted questions about the abstract parameters of the prior distribution and then used Bayesian data analysis to reconstruct the appropriate priors. %\gcs{how you estimated the state prior using targeting questions to determine parameters of the relevant distributions}.

\subsubsection{Utterance alternatives}

In some cases---for example, an analytic demonstration of a qualitative prediction---prior estimation might be unnecessary or infeasible. In such cases, one ought to make minimal assumptions, for example by assuming an uninformative, uniform prior. Better yet, the modeler may want to demonstrate the qualitative predictions under a number of different assumptions about the shape of the prior; in this case, Uniform, Gaussian, and Beta distributions are natural choices to explore. 
...
Like the specific probabilities assigned in the various priors, other free parameters of the model must be fixed in order to generate predictions. Free parameters common to RSA include $\alpha$, which controls speaker optimality, and the various utterance cost parameters. These parameters are often not of direct theoretical interest but sometimes can be informed by domain knowledge. For instance, it is often a natural assumption to assume to that the cost of an utterance is a function of either its length (which could be measured in terms of number of words, syllables, or characters) or its frequency (measured from a corpus). Unfortunately,  no comparable motivation exists for the setting of the speaker optimality parameter $\alpha$ and any effort to convince a participant that a speaker is ``not very rational'' may lead to unintended pragmatic inferences.
...
For the Vanilla RSA model, the number of layers of recursion can also be thought of as a free parameter. The rule of thumb has been that levels of recursion and speaker optimality trade-off with one another \mht{(Frank et al., manuscript; Peloquin et al.?  Levy cogsci 2018 paper?)} though recent analytic results argue against this simple picture \mht{(Zaslavsky, Hu, Levy, 2020 cogsci?}).
