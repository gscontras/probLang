...
Thus, \eqref{L0} returns a uniform probability distribution over the states $s$ that $u$ maps to \texttt{true}.\footnote{We are presenting a version of Vanilla RSA where $L_0$ does not take into account the state prior $P(s)$ in calculating the posterior over states; for more on this choice, including empirical justification, see \cite{qingfranke2015}. Some models may wind up using an informative prior at the level of $L_0$, as in the context inference models presented in Section \ref{context-inference} below.}
...
%\mht{haven't discussed $P(s)$ at this point, see previous note}. 
...
informativity. 
%\mf{I'm not sure if this paragraph needs to go here; it might have a better
...
%\mf{leave out that last sentence?} 
...
\mht{should the final term in the eqn above be: $P_{L_{0}\text{-}Bel}(s \mid u)$?}
...
Similar to the gradable adjectives model, the generics model adopts a threshold semantics where the generic is true just in case the prevalence of the feature $f$ within the category $k$ exceeds the relevant threshold \sem{generic}$^{\textbf{x}}$ = \lam f. \lam k. $P(f \mid k)$ $>$ \textbf{x}.
...
\cite{tesslergoodman2019} actually use a reduced version of this model where the threshold inference occurs at the level of the literal listener as opposed to the pragmatic listener. \cite{goodmanlassiter2015handbook} argued through simulation that threshold inference at the level of the literal listener results in interpretations that are too weak.
...
We focus in this section on issues in model development: the question of priors (broadly construed), linking functions between model predictions and empirical data, parameters external to RSA, and Bayesian data analysis. 

\subsection{Priors} \label{practicalities-prior}
...
RSA returns predictions in terms of the probability of either a state and/or other variables inferred by the listener (e.g., QUDs, thresholds, etc.) or an utterance (for speaker models). 
...
The fact that RSA is a Bayesian model opens the door to unpacking problems of adaptation and learning. 
...
Such convention formation is studied using iterated ``tangram'' experiments, where participant dyads take turns referring to abstract shapes (``tangrams'') with the intention of getting their partner to pick out the same referent. 
It is observed that over repeated rounds of this game, participants converge on conventional names for the referents, which are often reduced forms from how they originally refer to them (\cite{clark1986referring, hawkins2020characterizing}).
This  lexical adaptation process can be formalized in the RSA modeling framework through interlocutors' reasoning about each others' lexica, \emph{a la} a Meaning Inference model (\cite{hawkins2017convention}).
...
Using a model comparison approach within the RSA framework, the authors found that this kind of semantic/pragmatic adaptation is explained by listeners adapting their representations of the speaker’s lexicon (word--meaning mappings) and the utterances a speaker prefers to say. 

\subsection{Language development}

The RSA modeling framework puts forth a particular hypothesis about how language understanding unfolds, and the components and parameters of these can be taken to a correspond to a hypothesis about the representations that may vary across the population of speakers or develop with age. 
...
In naturalistic contexts, multiple information sources may be presenting, raising the question of how these sources are integrated.
