...
We focus on this section on three issues in model development: The question of priors (broadly construed), linking functions between model and empirical data, and parameters external to RSA. 

\subsection{Priors}
 
Here we focus on the question of priors for any parameter internal to the RSA model (as opposed to parameters external to the RSA model, which we be discussed below). 
These parameters include the states of the world over which the listener reasons, the utterances available to the speaker, and potentially a host of ``lifted variables'' which will vary according to the structural description of the model. 
Even prior to the questions of priors, the modeler must answer a basic representational question: What constitutes a world-state? At what level of linguistic description will we treat utterances?
The question of priors can be decomposed into at least two parts: (i) specifying the set of possible values of the variable (or, the support of the prior distribution; e.g., the set of world-states being considered) and (ii) assigning relative prior probabilities of each value in the set of possible values (e.g., the relative probability of each world-state). 

\subsubsection{Representation and possibilities}

One of the most important modeling decisions concerns setting up the conversational scenario: how do we represent the states of the world and utterances that can be described in our model? Answering these questions often entails specifying the universe of possibilities of states and utterances (i.e., the support of the prior distributions).
...
\mht{potential differences in the prior over states and the prior over meanings... i think FG12 is more about the prior over meanings, whereas Kao et al is about the prior on states... might we want a little discussion of when to think which?}
