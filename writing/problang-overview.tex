...
\end{lstlisting}
Notice that, for technical reasons, WebPPL indeed \emph{requires} the specification of an utterance prior even if we do not use it to implement salience!\footnote{WebPPL requires the inclusion of an utterance prior because it minimally needs to know the set of all available utterances. If no \emph{differential salience} is to be modelled, the utterance prior in a WebPPL program must still be specified but should then just be uniform.}
