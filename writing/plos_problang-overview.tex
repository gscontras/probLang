...
The current paper offers a practical introduction to the modeling framework, serving as a companion piece to the hands-on web-book at \href{https://www.problang.org}{www.problang.org} \cite{problang}. We begin with a high-level overview of RSA, walking through its basic implementation and the philosophical foundations that informed the architectural choices. We then explore variations to the basic architecture, surveying technological innovations that have allowed for broader empirical coverage. After a brief excursus on the relative benefits of ``thinking in math'' vs.~``thinking in code,'' we discuss common practical considerations facing the modeler. We then explore limitations of the current framework, which also serve as guidance for future extensions.
