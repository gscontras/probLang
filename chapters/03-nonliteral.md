...
> 3. Check the predictions for a speaker who paid 501 is aroused (valence is `true`) and wishes to only communicate valence. What's the three most likely messages such a speaker would send? (Hint: use `viz.table` to see all options ordered in terms of their probability; a histogram is not informative in this case.)
